...
function createListItem(blob, image, format, file) {
	var blobUrl = URL.createObjectURL(blob);
	var fileName = file.name;
	var regex = /ncm$/g;
	fileName = fileName.replace(regex, format);
	...
	for (var i=0; i<files.length; i++) {
		...
		reader.onloadend = (function(file) {
			return function(evt) {
				...
				var pack = Parse(arrayBuffer);
				createListItem(pack[0], pack[1], pack[2], file);
			};
		})(files[i]);
